...
public class ChatbotTest {
    ...
    private static final int COL_RESPONSE = 2; // Column where the chatbot answer will be placed
    ...
    private void initializeDriverAndLogin() {
        ...
        try {
            ...
            loginButton.click();                                                       
            // Fixed login wait issue: Checking for portal ID or alternatively locating the conversational AI section directly
    ...
    public void runAutomationFramework() {
        ...
        for (int i = 1; i < csvData.size(); i++) {      
            ...
            // Store the chatbot response into Column index 2 (Automation Answer)
            row[COL_RESPONSE] = chatbotResponse;            
            // Save results incrementally to your OneDrive folder path
            saveResultsToCsv(csvData);           
            if (i < csvData.size() - 1) {
                sleep(2000); 
            }
        }
        System.out.println("\nAll questions processed successfully and stored in OneDrive FrameworkQA.csv!");
    }
    ...
    private void saveResultsToCsv(List<String[]> csvData) {
        String filePath = "C:\\Users\\rames\\OneDrive\\Documents\\My Data Sources\\FrameworkQA.csv";
        try (PrintWriter writer = new PrintWriter(new OutputStreamWriter(new FileOutputStream(filePath), StandardCharsets.UTF_8))) {
            ...
            System.err.println("Error saving results to OneDrive FrameworkQA.csv: " + e.getMessage());
        }
    }
