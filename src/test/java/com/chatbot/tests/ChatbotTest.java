...
public class ChatbotTest {
    ...
    private static final int COL_RESPONSE = 6; 
            ...
            return longWait.until(driverInstance -> {
                ...
                if (currentResponses.size() > initialCount) {
                    ...
                    if (!isFiller && !text.equals(lastKnownText[0])) {
                        ...
                        stableCounter[0] = 0;
                    } else if (!isFiller && text.equals(lastKnownText[0]) && !text.isEmpty()) {
                        stableCounter[0]++;
                        if (stableCounter[0] >= 2) {
                            return text;
                        }
                    }
                }
    ...
    private List<TestRowData> fetchExcelDataFromGitHub(String fileUrl) {
        ...
        try (InputStream is = new URL(rawUrl).openStream();
            ...
            System.err.println("Failed to fetch or parse Excel from GitHub: " + e.getMessage());
        }
        return list;
    }
    private void saveResultsToExcel(String templatePath, List<TestRowData> dataList) {
        ...
        try (InputStream is = file.exists() ? new FileInputStream(file) : new URL(templatePath.replace("github.com", "raw.githubusercontent.com").replace("/blob/", "/")).openStream();
             Workbook workbook = new XSSFWorkbook(is)) {      
